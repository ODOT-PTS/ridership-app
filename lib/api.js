...
export const queryAgencies = (dataset) => {
  const db = openDb({ sqlitePath: untildify(dataset) })
  return getAgencies({}, ['agency_name'], undefined, { db })
}

export const queryStops = (dataset, { route_id, direction_id }) => {
  const db = openDb({ sqlitePath: untildify(dataset) })
  const values = []
  const whereClauses = []
...
export const queryRoutesAndDirections = (dataset) => {
  const db = openDb({ sqlitePath: untildify(dataset) })
...
const groupRidershipData = (dataset, boardAlights, filters) => {
  ...
  if (grouping === 'none') {
    ...
    const dataPoints = []
    const timeBucketSize = parseInt(time_bucket_size, 10) || 60
    ...
    const stops = queryStops(dataset, filters)
...
export const queryRidershipData = (dataset, filters) => {
  const db = openDb({ sqlitePath: untildify(dataset) })
  const {
    route_id,
    direction_id,
  ...
  return processRidershipData(
    groupRidershipData(dataset, boardAlights, filters),
  )
}

export const queryRidershipDateRange = (dataset) => {
  const db = openDb({ sqlitePath: untildify(dataset) })
