...
export const queryRoutesAndDirections = async () => {
  ...
  const routesWithDirections = await Promise.all(routes.map(async route => {
    ...
    const directionIds = uniqBy(trips, trip => trip.direction_id)
    const directions = await getDirections({ route_id: route.route_id })

    route.directions = directionIds.map(directionId => ({
      direction_id: directionId,
      direction: directions.find(direction => direction.direction_id === directionId)
    }))
