...
export const queryStops = async ({ route_id, direction_id }) => {
  ...
  if (
    ...
    try {
      ...
      const tripsWithStopCounts = await db.all(
    ...
    const stops = await getStops({ stop_id: routeStopOrder }, [
      'stop_id',
      'stop_name',
      'stop_lat',
      'stop_lon',
...
export const queryRoutesAndDirections = async () => {
  ...
  return sortBy(compact(routesWithDirections), (route) =>
...
const groupRidershipData = async (boardAlights, filters) => {
  ...
  if (grouping === 'none') {
    ...
    // Use average weekday
    const weekdayDataPoint = dataPoints.find(
      (dataPoint) => dataPoint.service_day_of_week_type === 'Weekday'
    )
    ...
    weekdayDataPoint.bike_boardings = Math.round(
      weekdayDataPoint.bike_boardings / 5
    )
    weekdayDataPoint.bike_alightings = Math.round(
      weekdayDataPoint.bike_alightings / 5
    )
    weekdayDataPoint.ramp_boardings = Math.round(
      weekdayDataPoint.ramp_boardings / 5
    )
    weekdayDataPoint.ramp_alightings = Math.round(
      weekdayDataPoint.ramp_alightings / 5
    )
...
export const queryRidershipData = async (filters) => {
  const { route_id, direction_id, stop_id, start_date, end_date, grouping } =
    filters
  const db = getDb()
  const values = []
  const whereClauses = []
